...
DDG_server <- function(input, output,session) {
  ...
  output$cp_DDG <- renderUI({
    ...
    if (input$submit_DDG > 0) {
      list(
        fluidRow(
          column(4,
                 textInput("xlab_ddg", "Enter required x-axis title", "X-axis")
          ),
          column(4,
                 textInput("ylab_ddg", "Enter required y-axis title", "Y-axis")
          ),
          column(4,
                 textInput("title_ddg", "Enter required title", "title")
          ),
          column(4,
                 selectInput("pal.col_DDG", "Choose Colour pattern (plot):",
      ...
      column(4,
             selectInput("label_col_ddg","choose color of the label",
                         c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                         "black")
      ),
      column(4,
             selectInput("type_ddg","choose type of the plot",
      ...
      column(5,
             sliderInput("size_ddg", "Change the label size:",
                         min = 0.5, max = 2, value = 0.8)
      ),
      column(5,
             sliderInput("width_ddg", "Change the line width:",
                         min = 0.5, max = 2, value = 0.8)
      ),
      column(5,
             sliderInput("groups_ddg", "Change the number of groups:",
                         min = 1, max = 10, value = 5)
      ),
          column(3,
                 materialSwitch(inputId = "Show_col_switch_DDG", label = "Show available colours", status = "danger")
          ) 
        )
  ...
  plotInput <- reactive({
    ...
    if (input$submit_DDG > 0) {
      ...
      dist_matrix <- dist( data_ddg)
      hclust_result <- hclust(dist_matrix)
      dend <- as.dendrogram(hclust_result)
       
      p<-fviz_dend(dend, cex=input$size_ddg, lwd=input$width_ddg, k=input$groups_ddg,
  ...
  ################### Download button
  output$image_down_ddg <- renderUI({
    if (is.null(input$submit_DDG)) {
      return()
    }
    if (input$submit_DDG > 0) {
      list(downloadButton("downloadImage14",
  ...
  output$downloadImage14 <- downloadHandler(
  ...
  ############################# download data set
  output$data_set_DDG = renderUI({
    
    list(
      selectInput(
        "filenames_ddg", "Choose a dataset:",
        list.files(
          pattern = c("Dendrogram 1.csv|Dendrogram 2.csv")
        )
      ),
      downloadButton("downloadData14", label = "Download csv file", class = "butt14",)
    )
