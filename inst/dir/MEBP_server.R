MEBP_server <- function(input, output,session) {
  ...
  ########################## MEBP 
  csvfile_MEBP <- reactive({
    csvfile_MEBP <- input$file1_MEBP
    if (is.null(csvfile_MEBP)){return(NULL)}
    dt_EBP  <- read.csv(csvfile_MEBP $datapath, header=input$header, sep=",")
    dt_EBP 
  })
  
  output$var_MEBP  <- renderUI({
    if(is.null(input$file1_MEBP$datapath)){
      return()
    }
    else{
      list (radioButtons("xmebp", "Select the x axis variables", choices =    names(csvfile_MEBP())),
            radioButtons("comebp", "Select the colouring variables", choices =    names(csvfile_MEBP())),
            radioButtons("ymebp", "Select the y axis variable", choices = names(csvfile_MEBP())),
            radioButtons("lmebp", "Select the significant letter", choices = names(csvfile_MEBP())),
            radioButtons("cmebp", "Select the error value (sd/se/ci)", choices = names(csvfile_MEBP())),
            actionBttn(
              inputId = "submit_MEBP",
              label = "DRAW!",
              color = "danger",
  ...
  output$cp_MEBP <- renderUI({
    ...
    if (input$submit_MEBP > 0) {
      list(
        fluidRow(
          column(4,
                 textInput("xlab_mebp", "Enter required x-axis title", "X-axis")
          ),
          column(4,
                 textInput("ylab_mebp", "Enter required y-axis title", "Y-axis")
          ),
          column(4,
                 textInput("title_mebp", "Enter required title", "title")
          ),
          column(4,
                 textInput("legend_mebp", "Enter required legend title", "legend")
          ),
          
          
          column(4,
                 selectInput("pal.col_MEBP", "Choose Colour pattern (plot):",
          ...
          column(4,
                 selectInput("Legend_Position_MEBP", "Choose legend position:",
          ...
          column(3,
                 materialSwitch(inputId = "Show_col_switch_MEBP", label = "Show available colours", status = "danger")
          ),
          
          column(
            4,
            materialSwitch(inputId = "manual_change_MEBP", label = "Show me manual controls", status = "danger")
          )
        )
    ...
    if (input$submit_MEBP > 0 && input$manual_change_MEBP > 0) {
      list(
        fluidRow(
          ...
          column(4,
                 selectInput("face_mebp", "select font face",
                             choices = c("plain","italic","bold","bold.italic"),
                             selected = "plain")
          ),
          column(4,
                 selectInput("font_selector_mebp", "select a font",
                             choices = c(
                             ...
                             selected = "Arial")),
          column(4,
                 selectInput("colour2_mebp","choose colour of axis title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 selectInput("colour4_mebp","choose colour of legend title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 selectInput("colour3_mebp","choose colour of plot title:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(5,
                 sliderInput("size1_mebp", "axis label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size2_mebp", "axis title size:",
                             min = 10, max = 20, value = 10
                 )
          ),
          column(5,
                 sliderInput("size3_mebp", "legend label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size4_mebp", "legend title size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size5_mebp", "plot title size:",
                             min = 15, max = 30, value = 15)
          ),
          column(5,
                 sliderInput("angle_mebp","required angle of x-axis labels (degrees)",min=0, max=135, value= 0, step = 15)
          ) 
        )
      )
    }
  ...
  plotInput <- reactive({
    ...
    if (input$submit_MEBP > 0) {
      ...
      if (is.null(input$pal.col_MEBP)){
        ...
        mycolors <- colorRampPalette(brewer.pal(8, input$pal.col_MEBP))(nb.col)
      }
  ...
  ###############color show
  output$colours_MEBP <- renderPlot( {
  ...
  ################### Download button
  output$image_down_mebp <- renderUI({
    if (is.null(input$submit_MEBP)) {
      return()
    }
    
    if (input$submit_MEBP > 0 ) {
      list(downloadButton("downloadImage4",
  ...
  output$downloadImage4 <- downloadHandler(
  ...
  output$data_set_MEBP = renderUI({
    ...
    list(
      selectInput(
        ...
        list.files(
          pattern = c("Multiple error bar 1.csv|Multiple error bar 2.csv")
        )
      ),
      downloadButton("downloadData4", label = "Download csv file", class = "butt4",)
    )
