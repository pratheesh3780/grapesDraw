...
MBP_server <- function(input, output,session) {
  ...
  output$var_MBP  <- renderUI({
    if(is.null(input$file1_MBP$datapath)){
      return()
    }
    else{
      list (radioButtons("xmbp", "Select the x axis variables", choices =    names(csvfile_MBP())),
            radioButtons("cmbp", "Select the colouring variable", choices = names(csvfile_MBP())),
            radioButtons("vmbp", "Select the observations (quantitative variable)", choices = names(csvfile_MBP())),
            actionBttn(
              inputId = "submit_MBP",
              label = "DRAW!",
              color = "danger",
  ...
  output$cp_MBP <- renderUI({
    ...
    if (input$submit_MBP > 0) {
      list(
        fluidRow(
          column(4,
                 textInput("xlab_mbp", "Enter required x-axis title", "X-axis")
          ),
          column(4,
                 textInput("ylab_mbp", "Enter required y-axis title", "Y-axis")
          ),
          column(4,
                 textInput("title_mbp", "Enter required title", "title")
          ),
          column(4,
                 textInput("legend_mbp", "Enter required legend title", "legend")
          ),
          
          
          column(4,
                 selectInput("pal.col_MBP", "Choose Colour pattern (plot):",
          ...
          column(4,
                 selectInput("Legend_Position_MBP", "Choose legend position:",
          ...
          column(3,
                 materialSwitch(inputId = "Show_col_switch_MBP", label = "Show available colours", status = "danger")
          ),
          column(
            4,
            materialSwitch(inputId = "manual_change_mbp", label = "Show me manual controls", status = "danger")
          )
        )
  ...
  output$manual_change_MBP <- renderUI({
    if (is.null(input$file1_MBP$datapath)) {
      return()
    }
    ...
    if (input$submit_MBP > 0 && input$manual_change_mbp > 0) {
      list(
        fluidRow(
          ...
          column(4,
                 selectInput("face_mbp", "select font face",
                             choices = c("plain","italic","bold","bold.italic"),
                             selected = "plain")
          ),
          column(4,
                 selectInput("font_selector_mbp", "select a font",
                             choices = c(
                             ...
                             selected = "Arial")),
          column(4,
                 selectInput("colour2_mbp","choose colour of axis title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 selectInput("colour4_mbp","choose colour of legend title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 selectInput("colour3_mbp","choose colour of plot title:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(5,
                 sliderInput("size1_mbp", "axis label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size2_mbp", "axis title size:",
                             min = 10, max = 20, value = 10
                 )
          ),
          column(5,
                 sliderInput("size3_mbp", "legend label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size4_mbp", "legend title size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 sliderInput("size5_mbp", "plot title size:",
                             min = 15, max = 30, value = 15)
          ),
          column(5,
                 sliderInput("angle_mbp","required angle of x-axis labels (degrees)",min=0, max=135, value= 0, step = 15)
          ),
          column(5,
                 sliderInput("Barwidth_mbp","select required bar width",
  ...
  plotInput <- reactive({
    ...
    if (input$submit_MBP > 0) {
      ...
      if (is.null(input$pal.col_MBP)){
        ...
        mycolors <- colorRampPalette(brewer.pal(8, input$pal.col_MBP))(nb.col)
      }
  ...
  ################### Download button
  output$image_down_mbp <- renderUI({
    if (is.null(input$submit_MBP)) {
      return()
    }
  ...
  output$data_set_MBP = renderUI({
    ...
    list(
      selectInput(
        ...
        list.files(
          pattern = c("multiple bar plot 1.csv|multiple bar plot 2.csv")
        )
      ),
      downloadButton("downloadData3", label = "Download csv file", class = "butt3",)
    )
