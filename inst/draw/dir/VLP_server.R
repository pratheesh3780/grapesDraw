VLP_server <- function(input, output,session) {
  ...
  ########################## VLP 
  csvfile_VLP <- shiny::reactive({
    csvfile_VLP <- input$file1_VLP
    if (is.null(csvfile_VLP)){return(NULL)}
    dt_MBP  <- read.csv(csvfile_VLP $datapath, header=input$header_VLP, sep=",")
    dt_MBP 
  })
  
  output$var_VLP  <- shiny::renderUI({
    if(is.null(input$file1_VLP$datapath)){
      return()
    }
    else{
      list (shiny::radioButtons("xvlp", "Select the x axis variables", choices =    names(csvfile_VLP())),
            shiny::radioButtons("yvlp", "Select the y axis variable", choices = names(csvfile_VLP())),
  ...
  output$cp_VLP <- shiny::renderUI({
    ...
    if (input$submit_VLP > 0) {
      list(
        fluidRow(
          column(4,
                 shiny::textInput("xlab_vlp", "Enter required x-axis title", "X-axis")
          ),
          column(4,
                 shiny::textInput("ylab_vlp", "Enter required y-axis title", "Y-axis")
          ),
          column(4,
                 shiny::textInput("title_vlp", "Enter required title", "title")
          ),
          column(4,
                 shiny::textInput("colour1_vlp", "Enter required legend title", "legend")
          ),
          
          
          column(4,
                 shiny::selectInput("pal.col_VLP", "Choose Colour pattern (plot):",
          ...
          column(4,
                 shiny::selectInput("Legend_Position_VLP", "Choose legend position:",
          ...
          column(3,
                 shinyWidgets::materialSwitch(inputId = "Show_col_switch_VLP", label = "Show available colours", status = "danger")
          ),
          column(
            4,
            shinyWidgets::materialSwitch(inputId = "manual_change_vlp", label = "Show me manual controls", status = "danger")
          )
        )
  ...
  output$manual_VLP <- shiny::renderUI({
    if (is.null(input$file1_VLP$datapath)) {
      return()
    }
    ...
    if (input$submit_VLP > 0 && input$manual_change_vlp > 0) {
      list(
        fluidRow(
          ...
          column(4,
                 shiny::selectInput("face_vlp", "select font face",
                             choices = c("plain","italic","bold","bold.italic"),
                             selected = "plain")
          ),
          column(4,
                 shiny::selectInput("font_selector_vlp", "select a font",
                             choices = c(
                             ...
                             selected = "Arial")),
          column(4,
                 shiny::selectInput("colour2_vlp","choose colour of axis title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 shiny::selectInput("colour4_vlp","choose colour of legend title and label:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(4,
                 shiny::selectInput("colour3_vlp","choose colour of plot title:",
                             choices = c("darkblue","red","blue","black","yellow","orange","purple","brown","cyan","magenta"),
                             selected = "black")
          ),
          column(5,
                 shiny::sliderInput("size1_vlp", "axis label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 shiny::sliderInput("size2_vlp", "axis title size:",
                             min = 10, max = 20, value = 10
                 )
          ),
          column(5,
                 shiny::sliderInput("size3_vlp", "legend label size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 shiny::sliderInput("size4_vlp", "legend title size:",
                             min = 10, max = 20, value = 10)
          ),
          column(5,
                 shiny::sliderInput("size5_vlp", "plot title size:",
                             min = 15, max = 30, value = 15)
          ),
          column(5,
                 shiny::sliderInput("angle_vlp","required angle of x-axis labels (degrees)",min=0, max=135, value= 0, step = 15)
          )
        )
      )
    }
  ...
  plotInput <- shiny::reactive({
    ...
    if (input$submit_VLP > 0) {
      ...
      if (is.null(input$pal.col_VLP)){
        ...
        mycolors <- colorRampPalette(RColorBrewer::brewer.pal(8, input$pal.col_VLP))(nb.col)
      }
  ...
  ###############color show
  output$colours_VLP <- shiny::renderPlot( {
  ...
  ################### Download button
  output$image_down_vlp <- shiny::renderUI({
    if (is.null(input$submit_VLP)) {
      return()
    }
    if (input$submit_VLP > 0) {
      list(shiny::downloadButton("downloadImage9",
  ...
  output$downloadImage9 <- shiny::downloadHandler(
  ...
  ############################# download data set
  output$data_set_VLP = shiny::renderUI({
    ...
    list(
      shiny::selectInput(
        ...
        list.files(
          pattern = c("violin_plot_1.csv|violin_plot_2.csv")
        )
      ),
      shiny::downloadButton("downloadData9", label = "Download csv file", class = "butt9",)
    )
  ...
  output$downloadData9 = shiny::downloadHandler(
    filename = function() {
      input$filenames_vlp
    },
    content = function(file) {
